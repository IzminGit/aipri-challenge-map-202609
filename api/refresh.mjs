...
export default async function handler(req, res) {
  ...
  try {
    const results = (await mapWithConcurrency(prefectures, 6, ([id, name]) => fetchPrefecture(id, name))).flat();
...
async function fetchPrefecture(id, prefecture) {
  ...
  for (let page = 2; page <= pages; page += 1) {
    htmls.push(await fetchHtml(buildUrl(id, page)));
  }
  return htmls
...
async function fetchHtml(url) {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'AipriChallengeMap202609/1.1',
        'Accept': 'text/html',
      },
...
function parseShop(block, prefecture, index) {
  ...
  return {
    ...
    address,
    prefecture,
    machineTypes: machineTypes(block),
...
function dlFields(block) {
  ...
  for (const match of block.matchAll(regex)) result[text(match[1])] = text(match[2]);
  return result;
}
...
function machineTypes(block) {
  return [
    ...
    ['verse', 'アイプリバース'],
    ['hiroba', 'アイプリステーション'],
  ].filter(([key]) => block.includes(`shopResultType__img--${key}`)).map(([, label]) => label);
}
